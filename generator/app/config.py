...
class Settings:
    ...
    @classmethod
    # cls : @classmethod로 정의 => Python이 자동으로 전달하는 클래스 자신을 의미  => Settings 클래스를 의미
    ...
    def from_env(cls) -> "Settings":
        ...
        if kinesis_enabled and not kinesis_stream_name:
            raise ValueError("KINESIS_STREAM_NAME값은 KINESIS_ENABLED=true일 때 필수 옵션입니다.")

        # 검증이 끝난 환경변수 값으로 Settings 객체 생성 -> 클래스에 인자 넣어서 객체 생성
        return cls(
            domain                          =domain,
